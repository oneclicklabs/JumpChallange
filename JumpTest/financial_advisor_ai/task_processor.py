...
class TaskProcessor:
    ...
    def stop(self):
        ...
        if self.thread:
            ...
            if self.thread.is_alive():
                logger.warning("Task processor thread did not terminate cleanly")
            self.thread = None
    ...
    def _process_tasks(self):
        ...
        for task in tasks:
            ...
            except Exception as e:                logger.error(f"Error processing task {task.id}: {str(e)}")
        ...
        for event in events:
            try:
                ...
                if service.process_webhook_event(event.id):
                    ...
                    logger.warning(f"Failed to process webhook event {event.id}")
