...
class CalendarAPI:
    ...
    def find_available_slots(self, date: datetime, duration_minutes: int = 30,
        ...
        try:
            ...
            end_of_day = datetime(date.year, date.month,
                                  date.day, end_hour, 0, 0)
        ...
        try:
            ...
            for event in events:
                ...
                if not existing.exists():
                    ...
                    start_time = datetime.fromisoformat(start_datetime.replace(
                        'Z', '+00:00')) if 'T' in start_datetime else datetime.strptime(start_datetime, '%Y-%m-%d')
                    end_time = datetime.fromisoformat(end_datetime.replace(
                        'Z', '+00:00')) if 'T' in end_datetime else datetime.strptime(end_datetime, '%Y-%m-%d')
...
def get_user_calendar_service(user):
    ...
    Returns:
        CalendarAPI instance or None if failed
    """
    try:
        calendar_api = CalendarAPI(user.id)
        if calendar_api.initialized:
            return calendar_api
        else:
            logger.error(
                f"Failed to initialize Calendar API for user {user.id}: {calendar_api.error}")
            return None
    except Exception as e:
        logger.error(
            f"Error getting Calendar service for user {user.id}: {str(e)}")
        return None


def sync_calendar_events(user, days=30):
    """Sync calendar events from Google Calendar to the database

    Args:
        user: Django User object
        days: Number of days to sync events for
    ...
    try:
        calendar_api = CalendarAPI(user.id)
        if calendar_api.initialized:
            # Get events and sync to database
            events = calendar_api.get_events(days)
            calendar_api.sync_events_to_db(days)
            return events
        else:
            logger.error(
                f"Failed to sync calendar events for user {user.id}: {calendar_api.error}")
            return []
    except Exception as e:
        logger.error(
            f"Error syncing calendar events for user {user.id}: {str(e)}")
        return []
