...
class AgentServiceTests(TestCase):
    ...
    def setUp(self):
        ...
        self.profile = UserProfile.objects.create(
            user=self.user,
            openai_api_key='test_api_key'  # Dummy API key for testing
        )
    ...
    def test_analyze_and_suggest_tasks(self, mock_openai):
        ...
        self.assertEqual(suggested_tasks[0].title, "Follow up with Test Contact")
...
class APIEndpointTests(TestCase):
    ...
    def test_approve_suggestion(self):
        """Test approving a suggested task"""
        url = reverse('api_approve_task_suggestion', args=[self.suggested_task.id])
...
class WebhookTests(TestCase):
    ...
    def test_gmail_webhook(self, mock_process):
        ...
        webhook_data = {
            'message': {
                'data': 'eyJpZCI6ImVtYWlsMTIzIiwic3VtbWFyeSI6IlRoaXMgaXMgYSB0ZXN0IGVtYWlsIn0=',  # Base64 encoded JSON
                'messageId': 'msg123'
            }
        }
...
class IntegrationTests(TestCase):
    ...
    def test_suggestion_generation_and_approval_flow(self, mock_openai):
        ...
        generate_response = self.client.post(reverse('api_generate_task_suggestions'))
