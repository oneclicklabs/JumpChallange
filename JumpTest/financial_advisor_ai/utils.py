...
class RAGService:
    ...
    def _combine_vector_stores(self):
        ...
        if self.email_vector_store and self.hubspot_vector_store:
            ...
            email_docs = self.email_vector_store.similarity_search("", k=100000)
            hubspot_docs = self.hubspot_vector_store.similarity_search("", k=100000)
            combined_docs = email_docs + hubspot_docs
            self.combined_vector_store = FAISS.from_documents(combined_docs, self.embeddings)
    ...
    def retrieve_related_contacts(self, name_query: str) -> List[Dict[str, Any]]:
        ...
        results = vector_store.similarity_search(f"contact name {name_query}", k=5)
        
        contacts = []
        for doc in results:
            if doc.metadata.get("source") in ["hubspot_contact", "hubspot_note"]:
                contact_id = doc.metadata.get("contact_id")
                contact_name = doc.metadata.get("name") or doc.metadata.get("contact_name")
    ...
    def answer_question(self, question: str, chat_history: List[Dict[str, str]], contact_id: Optional[str] = None) -> str:
        ...
        if contact_id:
            ...
            retriever = vector_store.as_retriever(search_kwargs={"filter": filter_by_contact})
        else:
